 ...
 *
 * Physical Memory Protection (SPMP) is RISC-V parlance for an MPU.
 *
 ...
 *
 * Thread-specific m-mode and u-mode SPMP entries start from the SPMP slot
...
/*
 * This is used to seed thread SPMP copies with global m-mode cfg entries
...
void z_riscv_spmp_stackguard_enable(struct k_thread *thread) {
    ...
    /* Activate our non-locked SPMP entries in m-mode */
    // csr_set(mstatus, MSTATUS_MPRV);S
}
...
#ifdef CONFIG_SPMP_STACK_GUARD
    /* Make sure m-mode SPMP usage is disabled before we reprogram it */
